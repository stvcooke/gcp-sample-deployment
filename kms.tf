...
resource "google_project_iam_binding" "kms" {
  ...
  # allowing access to the key ring should allow access to the key
  condition {
...
resource "google_kms_crypto_key_iam_binding" "buckets" {
...
## grants access for pubsub service to use kms
resource "google_kms_crypto_key_iam_binding" "pubsubs" {
  crypto_key_id = google_kms_crypto_key.this.id
  role          = data.google_iam_role.kms.name

  members = ["serviceAccount:service-${data.google_project.this.number}@gcp-sa-pubsub.iam.gserviceaccount.com"]
}